...
export function AdminWorkspace({ seo }: { seo: SeoSettings }) {
  ...
  return <><Navigation /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    <header className="flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900 p-6 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><Image src="/images/ai-agent-avatar.png" alt="Scout AI control assistant" width={64} height={64} className="rounded-2xl border border-cyan-700 object-cover" /><div><p className="text-xs font-semibold uppercase tracking-[.24em] text-cyan-400">Scout Control Center</p><h1 className="mt-1 text-3xl font-bold text-white">Admin workspace</h1><p className="mt-1 text-sm text-slate-400">Manage access and understand the organization footprint.</p></div></div><button onClick={signOut} className="rounded-lg border border-slate-700 px-3 py-2 text-sm text-slate-300 hover:border-red-800 hover:text-red-200">Sign out</button></header>
