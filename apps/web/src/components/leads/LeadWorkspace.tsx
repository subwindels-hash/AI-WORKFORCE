...
export function LeadWorkspace({ mode }: { mode: "discovery" | "pipeline" }) {
  ...
  const [searchMode, setSearchMode] = useState<"business" | "buyer">("business");
  const [buyerKeywords, setBuyerKeywords] = useState("crude oil buyer, crude oil importer, petroleum procurement, crude trading");
  const [buyerCountry, setBuyerCountry] = useState("Australia");
  const [buyerCity, setBuyerCity] = useState("");
  const [buyerTitles, setBuyerTitles] = useState("Crude Oil Buyer, Procurement Manager, Head of Crude Trading, Oil Trader");
  ...
  const search = async (event: FormEvent) => {
    ...
    const buyer = searchMode === "buyer";
    const split = (value: string) => value.split(/[,\n]+/).map(item => item.trim()).filter(Boolean);
    const keywords = buyer ? split(buyerKeywords) : undefined;
    const locationCountry = buyer ? buyerCountry.trim() || "Australia" : country.trim();
    const locationCity = buyer ? buyerCity.trim() : "";
    const searchQuery = buyer ? [...(keywords ?? []), locationCity, locationCountry].filter(Boolean).join(" ") : query;
    if (buyer && !keywords?.length) { setError("Enter at least one buyer keyword."); return; }
    setBusy(true); setError(""); setNotice(""); setSelected(new Set());
    try {
      const result = await leadDiscoveryApi.search({
        query: searchQuery,
        mode: buyer ? "buyer" : "business",
        provider: buyer ? "apollo_io" : provider,
        limit: 20,
        ...(buyer ? { country: locationCountry, ...(locationCity ? { city: locationCity } : {}), keywords, titles: split(buyerTitles), verifiedEmailOnly: true, workEmailOnly: true, emailPolicy: "verified_work_email" as const } : { ...(category ? { category } : {}), ...(country ? { country } : {}) }),
      }, token);
      setLeads(result.results);
      setNotice(buyer
        ? `${result.verifiedEmailCount ?? result.results.length} provider-verified work-email buyer leads · ${result.newLeadsCreated} new · ${result.duplicatesDetected} duplicate signals${result.notice ? ` — ${result.notice}` : ""}`
        : `${result.results.length} businesses returned · ${result.newLeadsCreated} new leads · ${result.duplicatesDetected} duplicate signals`);
  ...
  return <>
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      {mode === "discovery" && <section className="mb-5 rounded-xl border border-slate-800 bg-slate-900 p-4">
        <div className="mb-4 flex flex-wrap gap-2" role="tablist" aria-label="Lead discovery mode">
          <button type="button" onClick={() => setSearchMode("business")} className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${searchMode === "business" ? "border-cyan-400 bg-cyan-400 text-slate-950" : "border-slate-700 text-slate-300"}`}>Business search</button>
          <button type="button" onClick={() => { setSearchMode("buyer"); setProvider("apollo_io"); }} className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${searchMode === "buyer" ? "border-cyan-400 bg-cyan-400 text-slate-950" : "border-slate-700 text-slate-300"}`}>Verified crude-oil buyer emails</button>
        </div>
        {searchMode === "buyer" ? <form onSubmit={search} className="grid gap-3 lg:grid-cols-[1fr_180px_220px_auto]">
          <label className="text-xs uppercase tracking-wide text-slate-400">Buyer keywords<input value={buyerKeywords} onChange={event => setBuyerKeywords(event.target.value)} aria-label="Buyer keywords" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white" /></label>
          <label className="text-xs uppercase tracking-wide text-slate-400">Country<input value={buyerCountry} onChange={event => setBuyerCountry(event.target.value)} aria-label="Buyer country" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white" /></label>
          <label className="text-xs uppercase tracking-wide text-slate-400">City (optional)<input value={buyerCity} onChange={event => setBuyerCity(event.target.value)} placeholder="Perth, Sydney…" aria-label="Buyer city" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white" /></label>
          <button disabled={busy || !token} className="self-end rounded-lg bg-cyan-400 px-5 py-2.5 font-semibold text-slate-950 disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Searching…" : "Find verified emails"}</button>
          <label className="text-xs uppercase tracking-wide text-slate-400 lg:col-span-3">Decision-maker titles<input value={buyerTitles} onChange={event => setBuyerTitles(event.target.value)} aria-label="Buyer titles" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white" /></label>
          <p className="text-xs leading-5 text-slate-500 lg:col-span-4">Strict mode: Apollo must return a usable address with an explicit provider-verified status. No guessed, generated, masked or placeholder emails are saved. Contact reveal is administrator-controlled and can spend provider credits.</p>
        </form> : <form onSubmit={search} className="grid gap-3 lg:grid-cols-[1fr_180px_180px_auto]">
          <label className="text-xs uppercase tracking-wide text-slate-400">Business search<input value={query} onChange={event => setQuery(event.target.value)} aria-label="Business query" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white outline-none focus:border-cyan-400" /></label>
          <label className="text-xs uppercase tracking-wide text-slate-400">Category<input value={category} onChange={event => setCategory(event.target.value)} placeholder="Restaurants" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white" /></label>
          <label className="text-xs uppercase tracking-wide text-slate-400">Country<input value={country} onChange={event => setCountry(event.target.value)} placeholder="Nigeria" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white" /></label>
          <label className="text-xs uppercase tracking-wide text-slate-400">Provider<select value={provider} onChange={event => setProvider(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white" disabled={!providers.length}>{providers.length ? providers.map(item => <option key={item.name} value={item.name}>{item.name} · {item.health.status}</option>) : <option>Loading providers…</option>}</select></label>
          <button disabled={busy || !token} className="self-end rounded-lg bg-cyan-400 px-5 py-2.5 font-semibold text-slate-950 disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Searching…" : "Search businesses"}</button>
          <p className="mt-1 text-xs text-slate-500 lg:col-span-5">Results are saved to PostgreSQL and deduplicated by provider plus stable source ID. No synthetic businesses are used.</p>
        </form>}
      </section>}
