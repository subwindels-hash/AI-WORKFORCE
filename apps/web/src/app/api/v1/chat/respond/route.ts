...
const localGuide = (message: string) => {
  ...
  if (value.includes("trading") || value.includes("paper") || value.includes("broker")) return "Trading starts in ANALYSIS_ONLY, and the kill switch ships released because it is scoped: engaging it blocks only broker and trading-intelligence order paths (MT5, MT4, crypto, forex, stock/ETF connectors, execution supervisor, trading intelligence, paper orders) and never sports, lottery, languages, leads, multiplier, messages or market-data reads. Use /paper for simulation, /strategy for research, /execution for governed routing, /brokers for connectors, and /risk for limits. Agents never bypass risk controls or place broker orders directly.";
