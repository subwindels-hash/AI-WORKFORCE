...
export async function discoveryRoutes(app: FastifyInstance): Promise<void> {
  app.post("/search", async (request) => {
    ...
    try {
      ...
      const repository = new LeadRepository(app.db); const saved = await Promise.all(businesses.map(business => repository.upsertDiscovery(principal.organizationId, provider.name, business))); const created = saved.filter(item => item.created).length;
      await repository.recordSearch({ organizationId: principal.organizationId, userId: principal.sub, query: input.query, provider: provider.name, limit: input.limit, resultsReturned: businesses.length, newLeadsCreated: created, duplicatesDetected: businesses.length - created, durationMs: Math.round(performance.now() - started) });
      return { provider: provider.name, providerStatus: health.status, results: saved.map(item => item.lead), newLeadsCreated: created, duplicatesDetected: businesses.length - created, duplicateCandidatesCreated: 0 };
    } catch (error) {
