...
import { z } from 'zod';
import {
  IMPLEMENTED_TRADING_MODES, INTEGRATION_STATUS, MARKET_CLASS_SYMBOLS, TRADING_MODES,
} from '@aegis/core';
import type { MarketClass, Timeframe, TradingIntelligenceEngine, ProviderManager, RiskEngine, EventBus } from '@aegis/core';
...
export function inferMarketClass(symbol: string): MarketClass {
...
export function registerCoreRoutes(app: FastifyInstance, deps: CoreDeps): void {
  const { engine, providerManager, riskEngine, eventBus } = deps;
  ...
  app.get('/api/market-data/candles', async (req, reply) => {
    const q = z
      .object({
        ...
        limit: z.coerce.number().int().min(30).max(5000).default(200),
  ...
  app.post('/api/agents/consensus', async (req, reply) => {
    ...
    if (!requests) {
      const symbols = parsed.data.symbols ?? ['EURUSD', 'GBPUSD', 'USDJPY', 'XAUUSD', 'BTCUSDT', 'ETHUSDT', 'SOLUSDT'];
