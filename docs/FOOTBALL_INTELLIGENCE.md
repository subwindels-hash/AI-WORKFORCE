...
| `/sports`, `/sports/tickets` | the match odds record engine only. They link to `/football` for football figures and never render them |
...
`MODEL_NOT_CALIBRATED` remains the match odds record engine's decision code
