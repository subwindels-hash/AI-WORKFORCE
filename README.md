...
| **Football Intelligence**: provider-gated fixture/statistics sync (api-football / thesportsdb / sportmonks), Poisson/Dixon–Coles scoreline model, data-quality gate with provenance, A/B/C categories configurable from Admin (thresholds + labels + enable/disable), Odds Prediction Ticket (kickoff-ordered, filterable, never a guarantee), match analysis console, live board, settlement + 30-day performance (outcome/exact-score/confidence/calibration, by category and by model), read-only backtest over stored history, model lifecycle (draft → approval → activation), RBAC (sports.*), idempotent football-cron | **TESTED** (console at /football, ticket at /football/ticket, admin at /admin/football; every figure stored from provider data — no synthetic rows) |
...
**897 automated tests** run through the real CodeIgniter stack
