...
class SportsRepositoryStub implements \AIWorkforce\Persistence\SportsRepository
{
    ...
    public function saveMatch(int $providerId, array $m): array
    {
        ...
        // Break the foreach reference first: without this, `$row = …` below
        // wrote THROUGH the reference and silently replaced the last stored
        // match row on every insert (the new row then appeared twice and the
        // previous match lost its odds/predictions).
        unset($row);
