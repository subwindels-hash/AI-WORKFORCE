...
test('sports UI: dashboard renders the honest DISABLED_NO_PROVIDER state', function () {
    ...
    assert_contains('Sports Intelligence — match odds record engine', $html);
...
test('sports UI: dashboard renders today match odds record with gated actions', function () {
    $repo = new SportsRepositoryStub();
...
test('sports UI: records console renders records, runs and performance', function () {
    ...
    assert_true(!str_contains($html, 'No match odds records generated yet'), 'seeded record must be listed, not the empty state');
});
...
test('sports UI: provider identities and diagnostics are hidden from read-only users', function () {
    ...
    assert_contains('NO RECORD — DATA_UNAVAILABLE', $user);
