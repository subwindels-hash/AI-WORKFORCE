...
test('configuration returns safe defaults before any admin change', function () {
    ...
    assert_equals(70.0, (float) $c['min_confidence']);
...
test('configuration validation rejects malformed values', function () {
    ...
    assert_true($svc->update(['min_confidence' => 70.0], 'a', 'allow 70 percent floor')['ok']);
    assert_false($svc->update(['min_confidence' => 69.99], 'a')['ok']);
