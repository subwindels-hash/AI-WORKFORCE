...
test('api provider schema, routes, views and catalog are installed', function () {
    ...
    $services = \Aegis\ApiProviders::services();
    ...
    assert_false(str_contains($admin, '\\\\Aegis\\\\ApiProviders'), 'Admin controller must not double-escape the store class');
});
...
test('api secrets encrypt at rest, mask in listings and never appear in dashboard or audit-shaped payloads', function () {
    ...
    $saved = \Aegis\ApiProviders::save($db, [
    ...
    try {
        ...
        $opened = \Aegis\ApiProviders::open($raw['secret_blob']);
        assert_contains($secret, $opened);
        assert_equals(\Aegis\ApiProviders::mask($secret), '••••••••••••9999');

        $listed = \Aegis\ApiProviders::list($db);
        ...
        $dash = json_encode(\Aegis\ApiProviders::dashboard($db));
        assert_false(str_contains($dash, $secret));
        $dashRows = \Aegis\ApiProviders::dashboard($db);
        $lead = null;
        ...
        $secrets = \Aegis\ApiProviders::findSecrets($db, $id);
...
test('saving a new primary provider demotes the previous primary for that service only', function () {
    ...
    try {
        ...
        $active = \Aegis\ApiProviders::activeConfig($db, 'translation');
...
test('save keeps existing secrets when the new secret is blank or the actor cannot change credentials', function () {
    ...
    $id = (int) $saved['id'];
    try {
        \Aegis\ApiProviders::save($db, [
            'service' => 'llm', 'driver' => 'openai_compatible', 'label' => 'LLM',
            'role' => 'primary', 'enabled' => 1,
            'base_url' => 'https://api.example/v1/chat/completions',
            'api_key' => '', 'model' => 'demo-model',
        ], $id, 1, true);
        assert_equals('keep-me-key-4242', \Aegis\ApiProviders::findSecrets($db, $id)['api_key'] ?? null);
        ...
        assert_equals('keep-me-key-4242', \Aegis\ApiProviders::findSecrets($db, $id)['api_key'] ?? null, 'no-credentials actor cannot rotate secrets');
    } finally {
        fx_api_cleanup([$id]);
    }
});
...
test('base URL must be HTTPS and unknown pairings are rejected', function () {
    $db = fx_api_db();
    assert_throws(InvalidArgumentException::class, fn () => \Aegis\ApiProviders::save($db, [
...
test('Test Connection talks to the provider through the injectable HTTP client and never echoes secrets', function () {
    ...
    $seen = [];
    try {
        \Aegis\ApiProviders::$http = function (string $url, array $headers = [], ?string $body = null) use (&$seen) {
            $seen[] = $url;
            return ['status' => 200, 'body' => '{}'];
        };
        $ok = \Aegis\ApiProviders::test($saved, []);
        assert_true($ok['ok']);
        ...
        \Aegis\ApiProviders::$http = function (string $url, array $headers = [], ?string $body = null) {
            return ['status' => 401, 'body' => 'invalid api key sk-abcdefghijklmnop'];
        };
        $places = ['driver' => 'google_places', 'base_url' => '', 'extra' => []];
        $fail = \Aegis\ApiProviders::test($places, ['api_key' => 'sk-abcdefghijklmnop']);
...
test('publicError never leaks key, token, env or configuration internals to members', function () {
    $safe = \Aegis\ApiProviders::USER_UNAVAILABLE;
    foreach ([
        ...
        assert_equals($safe, \Aegis\ApiProviders::publicError($internal), $internal);
        assert_false(str_contains(\Aegis\ApiProviders::publicError($internal), 'API key'));
    }
...
test('Google Places, lottery and sports resolve managed credentials from the store', function () {
    ...
    $ids = [];
    try {
        $places = \Aegis\ApiProviders::save($db, [
        ...
        $lotto = \Aegis\ApiProviders::save($db, [
...
test('chat assistant and language voice-status consume the managed store without exposing secrets', function () {
    ...
    try {
        ...
        $out = (new \Aegis\ChatAssistant())->respond('How do I start language learning?');
        ...
        $status = \Aegis\ApiProviders::publicStatus('llm');
...
test('configuring crypto market data does not authorize trading execution', function () {
    $db = fx_api_db();
    $market = \Aegis\ApiProviders::save($db, [
        'service' => 'crypto_market', 'driver' => 'binance_public', 'label' => 'Market only',
        'role' => 'primary', 'enabled' => 1, 'base_url' => 'https://api.binance.com',
    ], null, 1, true);
    try {
        assert_true(\Aegis\ApiProviders::serviceEnabled($db, 'crypto_market', false));
        assert_null(\Aegis\ApiProviders::activeConfig($db, 'trading_execution'));
        assert_false(\Aegis\ApiProviders::serviceEnabled($db, 'trading_execution', false), 'execution stays off until explicitly configured');
        $services = \Aegis\ApiProviders::services();
        assert_equals('data', $services['crypto_market']['kind']);
        assert_equals('action', $services['trading_execution']['kind']);
    } finally {
...
test('admin API RBAC is explicit: view/test vs manage/credentials', function () {
    assert_in_array('admin.api.view', array_keys(AEGIS_RBAC_PERMISSIONS));
    assert_in_array('admin.api.manage', array_keys(AEGIS_RBAC_PERMISSIONS));
    assert_in_array('admin.api.test', array_keys(AEGIS_RBAC_PERMISSIONS));
    assert_in_array('admin.api.credentials', array_keys(AEGIS_RBAC_PERMISSIONS));
    assert_in_array('admin.api.view', AEGIS_RBAC_GRANTS['admin']);
    assert_in_array('admin.api.test', AEGIS_RBAC_GRANTS['admin']);
    assert_false(in_array('admin.api.manage', AEGIS_RBAC_GRANTS['admin'], true));
    assert_false(in_array('admin.api.credentials', AEGIS_RBAC_GRANTS['admin'], true));
    assert_false(in_array('admin.api.view', AEGIS_RBAC_GRANTS['support_admin'], true));
    foreach (['admin.api.view', 'admin.api.manage', 'admin.api.test', 'admin.api.credentials'] as $perm) {
        assert_in_array($perm, AEGIS_RBAC_GRANTS['super_admin']);
    }
