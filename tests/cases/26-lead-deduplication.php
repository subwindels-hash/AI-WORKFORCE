...
test('secondary deduplication does not create a candidate without strong matching data', function () {
    $db=ci()->db;$now=gmdate('c');foreach ([['id'=>'dedupe-c','source_id'=>'source-c','name'=>'Alpha','phone'=>null],['id'=>'dedupe-d','source_id'=>'source-d','name'=>'Beta','phone'=>null]] as $lead)$db->insert('leads',$lead+['organization_id'=>'dedupe-no-match','source'=>'google_places','status'=>'new','metadata'=>'{}','created_at'=>$now,'updated_at'=>$now]);
