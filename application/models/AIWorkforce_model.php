    ...
    public function __construct()
    {
        ...
        $this->sports = new class($db) implements AIWorkforce\Persistence\SportsRepository {
            ...
            public function saveResult(int $matchId, int $providerId, array $r): void {
                $row=$this->db->get_where('sports_results',['match_id'=>$matchId,'provider_id'=>$providerId],1)->row_array();
                // A re-observed result that states exactly what the stored row
                // already states is the SAME observation: it must not wipe a
                // verification an operator or the settlement sweep already
                // made. A result that CHANGED (a score correction) is a new
                // claim — it resets to unverified and must earn verification
                // again before anything settles from it.
                $unchanged = $row !== null
                    && (string) $row['status'] === (string) $r['status']
                ...
                $data=['home_score'=>$r['homeScore'],'away_score'=>$r['awayScore'],'status'=>$r['status'],'verified'=>$keepVerified?1:0,'source_timestamp'=>$r['sourceTimestamp'],'verified_at'=>$keepVerified?($row['verified_at'] ?? null):null,'payload'=>json_encode($r['payload'])];
                if($row)$this->db->where('id',$row['id'])->update('sports_results',$data); else $this->db->insert('sports_results',array_merge(['match_id'=>$matchId,'provider_id'=>$providerId],$data)); }
            /** Null-aware score equality: null and 0 are DIFFERENT claims (a VOID/POSTPONED result carries no score). */
