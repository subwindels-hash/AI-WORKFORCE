  ...
  <div class="football-match-layout">
    <div class="stack">
      ...
      <section class="panel football-section football-match-odds" aria-labelledby="all-odds-heading">
        ...
        <div class="body">
          <p class="football-section-intro">Every market WINDELS can answer for this match is shown below, at <b>every line it is quoted at</b> — match result, double chance, draw no bet, winning margin, result-and-BTTS, the goal ladder from 0.5 to 6.5, odd/even, goal bands, per-team goal ladders, clean sheets, the full correct-score board, a nineteen-rung Asian handicap ladder including quarter lines, and the first- and second-half markets. Each rung is a different bet with its own probability and its own price, and every one of them is summed from the same stored score grid — one model read many ways, not many models. Where the provider quoted a price, the sheet includes its timestamp and source. Where it did not, the line remains <b>UNPRICED</b> with its model probability still shown. Provider-price-only markets such as corners, cards and HT/FT never receive a made-up WINDELS probability.</p>
          <?php if ($markets === []): ?>
