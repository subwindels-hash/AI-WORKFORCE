...
/** AI specialist backed by the configured provider. Domain facts must be
 * supplied by the core services; the model is never treated as a
 * market/sports authority. */
final class CloudflareSpecialistAgent implements SpecialistAgent
{
    ...
    public function handle(array $request, array $context): array
    {
        ...
        if (!is_array($cfg)) return ['status'=>'UNAVAILABLE','reason'=>'No AI provider is configured'];
        ...
        return $answer===null?['status'=>'UNAVAILABLE','reason'=>'AI model request failed']:['status'=>'COMPLETED','role'=>$this->role,'answer'=>$answer,'tools'=>$this->allowedTools];
    }
}
