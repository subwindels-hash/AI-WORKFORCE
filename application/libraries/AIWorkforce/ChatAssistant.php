...
TXT;
    ...
    public function localAnswer(string $message, ?array $user = null): string
    {
        ...
        return match ($topic) {
            ...
            'trading' => 'Trading defaults to ANALYSIS_ONLY + kill switch ON. Paper Trading (/paper) simulates orders through FULL risk chain. Strategy Lab (/strategy) backtests, requires paper evidence before live approval. My Trading (/app/trading) shows positions. Agents analyze; never place broker orders. AUTOMATIC KILL SWITCH: there is no manual switch anywhere. Protection monitors high-impact news, daily loss, drawdown, broker/MT4/MT5 connectivity, market data, spread, slippage and order failures, and blocks new trades by itself. It gates the execution supervisor, broker orders (MT5/MT4/crypto/forex), paper orders and automation modes; market data keeps streaming and non-trading modules (sports, lottery, language, leads) are never gated. Status is on My Trading and Risk Center (/risk); thresholds are configured by an administrator at /admin/protection.',
            ...
            'risk' => 'Risk Center (/risk) — limits, Automatic Kill Switch status and reason (no manual switch exists), Portfolio Risk Monitor (exposure, leverage, correlated positions, drawdown, daily loss, broker disconnect). Risk Engine can veto any paper/live order. Default boot: ANALYSIS_ONLY + kill switch active.',
