...
TXT;
    ...
    public function localAnswer(string $message, ?array $user = null): string
    {
        ...
        return match ($topic) {
            ...
            'trading' => 'Trading defaults to ANALYSIS_ONLY + kill switch ON. Paper Trading (/paper) simulates orders through FULL risk chain. Strategy Lab (/strategy) backtests, requires paper evidence before live approval. My Trading (/app/trading) shows positions. Agents analyze; never place broker orders. KILL SWITCH SCOPE: it is a broker + trading safeguard only — it blocks the execution supervisor, broker orders (MT5/MT4/crypto/forex), paper orders and automation modes. Market data keeps streaming and non-trading modules (sports, lottery, language, leads) are never gated. Engage/release it on My Trading or Risk Center (/risk).',
            ...
            'risk' => 'Risk Center (/risk) — limits, kill switch (scoped to broker + trading intelligence: execution, broker orders, paper orders, automation modes), Portfolio Risk Monitor (exposure, leverage, correlated positions, drawdown, daily loss, broker disconnect). Risk Engine can veto any paper/live order. Default boot: ANALYSIS_ONLY + kill switch active.',
