...
CREATE TABLE IF NOT EXISTS user_broker_connections (id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT NOT NULL, broker VARCHAR(40) NOT NULL, label VARCHAR(120) NULL, base_url VARCHAR(255) NOT NULL, extra_url VARCHAR(255) NULL, token_ciphertext TEXT NULL, token_nonce VARCHAR(64) NULL, account_hint VARCHAR(120) NULL, enabled TINYINT(1) NOT NULL DEFAULT 0, trading_enabled TINYINT(1) NOT NULL DEFAULT 0, live_allowed TINYINT(1) NOT NULL DEFAULT 0, last_test_ok TINYINT(1) NULL, last_test_message VARCHAR(255) NULL, last_test_at VARCHAR(32) NULL, created_at VARCHAR(32) NOT NULL, updated_at VARCHAR(32) NOT NULL, UNIQUE KEY uq_user_broker (user_id, broker), KEY idx_user_enabled (user_id, enabled)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
