...
class Api_system extends Api_controller
{
    public const FEATURES = [
        ...
        ['name' => 'Live market data after connecting (activate + in-process refresh)', 'category' => 'market-data', 'status' => 'TESTED', 'detail' => 'GET /api/market-data/refresh rebuilds the provider chain and reports configured-vs-live per service; php index.php tools marketdata --activate promotes a keyless public feed (Binance / Frankfurter) that an operator already saved, audited as MARKET_DATA_ACTIVATED. Never enables custom_http, a licensed feed, anything needing a credential, or a service already live'],
        ['name' => 'Live streaming candlestick chart', 'category' => 'market-data', 'status' => 'TESTED', 'detail' => 'GET /api/market-data/live streams candles + quote + a live verdict; assets/js/market-chart.js redraws the server SVG (candles, volume, EMA20/50, S/R + setup overlays). The LIVE badge is earned from provenance: non-synthetic AND fresh AND undelayed, otherwise SIMULATION / STALE / DELAYED. Synthetic data never auto-refreshes; polling pauses on a hidden tab and stops after 4 failures'],
        ['name' => 'Public delayed stock/ETF/futures data (Yahoo chart)', 'category' => 'market-data', 'status' => 'IMPLEMENTED', 'detail' => 'Allow-listed US equities, ETFs and a small futures set via the public Yahoo Finance chart endpoint. DELAYED, not licensed, not a live-trading feed. Native intervals 1m/5m/15m/1h/1d; 4h is refused rather than synthesized. Falls back to the labeled synthetic provider when unreachable'],
        ['name' => 'Stock/ETF/futures/options licensed data providers', 'category' => 'market-data', 'status' => 'PLANNED', 'detail' => 'Provider-neutral licensed-feed adapters are scaffolded and registered (HTTPS, license metadata, symbol allow-list, health ok:true); production remains PLANNED until a licensed source is verified. Prefer these over Yahoo when enabled'],
        // agents
        ...
        ['name' => 'Kill switch (scoped to broker + trading intelligence)', 'category' => 'engine', 'status' => 'TESTED', 'detail' => 'Ships RELEASED in DB state and governs ORDER PATHS only: broker connectors (MT5, MT4, cryptocurrency exchanges, forex/stock brokers, per-user connections), the Trade Execution Supervisor, the Trading Intelligence Engine risk decision and paper order placement. Engaging it never blocks Sports, Football, EuroMillions, Language Learning, Lead Discovery, Multiplier AI, Messages, Notifications or the workforce agents, and never blocks read-only market data (Crypto / Forex / Stock-ETF-Futures feeds, charts, quotes). Position close, order cancel and settlement stay available so exposure can always be unwound'],
    ...
    public function status()
    {
        ...
        $this->json([
            ...
            // The switch is scoped: it governs broker + trading-intelligence
            // order paths and nothing else (KillSwitchPolicy).
            'killSwitchScope' => $this->platform->killSwitchScope(),
    ...
    public function kill_switch()
    {
        ...
        $this->json(['killSwitch' => $ks, 'scope' => $this->platform->killSwitchScope()]);
    }
