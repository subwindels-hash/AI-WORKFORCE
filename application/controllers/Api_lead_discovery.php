...
class Api_lead_discovery extends Api_controller
{
    ...
    public function search() {
        ...
        if($error)return $this->jsonError($error,503,['providerStatus'=>$providerStatus]); $this->json(['provider'=>$providerName,'providerStatus'=>$providerStatus,'results'=>$results,'newLeadsCreated'=>$new,'duplicatesDetected'=>$dupes+$candidateCount,'duplicateCandidatesCreated'=>$candidateCount]);
    }
    ...
    public function duplicates($resolve=false){
        ...
        if(!$resolve)return $this->json(['duplicates'=>$this->db->where(['organization_id'=>$this->org,'status'=>'open'])->order_by('created_at','DESC')->get('duplicate_candidates')->result_array()]);
