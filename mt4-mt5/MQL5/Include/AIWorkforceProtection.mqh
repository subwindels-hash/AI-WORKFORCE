      ...
      if(percentBreach || fixedBreach)
        {
         if(fixedBreach)
            return(AIWF_Set(AIWF_KILL, "EA_DAILY_LOSS_LIMIT",
                   StringFormat("Daily loss %.2f reached the fixed limit of %.2f.", loss, limitFixed)));
         return(AIWF_Set(AIWF_KILL, "EA_DAILY_LOSS_LIMIT",
                StringFormat("Daily loss reached %.2f%% of equity (limit %.2f%%).", lossPct, limitPercent)));
        }
   ...
   if(drawdownLimit > 0.0 && drawdown > 0.0)
     {
      if(drawdown >= drawdownLimit)
         return(AIWF_Set(AIWF_KILL, "EA_MAX_DRAWDOWN",
                StringFormat("Drawdown %.2f%% reached the %.2f%% limit.", drawdown, drawdownLimit)));
      if((drawdown / drawdownLimit) >= 0.8)
         AIWF_Set(AIWF_WARNING, "EA_DRAWDOWN_APPROACHING",
                  StringFormat("Drawdown at %.0f%% of the configured maximum.", MathMin(100.0, drawdown / drawdownLimit * 100.0)));
     }
